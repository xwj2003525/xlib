...
static constexpr uint8_t days_in_month[13] = { 0,31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
...
constexpr bool x::time::isValidDateTime(int year, int month, int day, int hour, int minute, int second, int millisecond)
{
    ...
    return (2000 <= year&&year<=2999)&& (1 <= day && day <= max_days) && (0 <= hour && hour <= 23) && (0 <= minute && minute <= 59) && (0 <= second && second <= 59) && (0 <= millisecond && millisecond <= 999);
}
...
{
    std::ostringstream oss; oss << static_cast<uint32_t>(day) << " " << std::setw(2) << std::setfill('0') << static_cast<uint16_t>(hour) << ":" << std::setw(2) << std::setfill('0') << static_cast<uint16_t>(minute) << ":" << std::setw(2) << std::setfill('0') << static_cast<uint16_t>(second) << "." << std::setw(3) << std::setfill('0') << millisecond; return oss.str();
